...
const aeroflyConfigFile = function(filename) {
  ...
  /**
   * Get wetaher data from config file
   * @return {Object} AeroflyObject
   */
  _public.getAeroflyObject = function() {
    let aeroflyValues = {};
    ...
    // restrict this to `tm_time_utc`
    let timeMatch = _private.getGroup(configFileContent, 'tm_time_utc', 2);
    if (timeMatch) {
      aeroflyValues.time = {};
      aeroflyValues.time.year   = Number(_private.getValue(timeMatch, 'time_year'));
      aeroflyValues.time.month  = Number(_private.getValue(timeMatch, 'time_month'));
      aeroflyValues.time.day    = Number(_private.getValue(timeMatch, 'time_day'));
      aeroflyValues.time.hours  = Number(_private.getValue(timeMatch, 'time_hours'));
      aeroflyValues.time.hour   = Math.floor(aeroflyValues.time.hours);
      aeroflyValues.time.minute = Math.floor(aeroflyValues.time.hours % 1 * 60);
    }

    aeroflyValues.visibility = Number(_private.getValue(configFileContent, 'visibility'));

    // restrict this to `tmsettings_wind`
    let windMatch = _private.getGroup(configFileContent, 'tmsettings_wind', 2);
    if (windMatch) {
      aeroflyValues.wind = {};
      aeroflyValues.wind.strength    = Number(_private.getValue(windMatch, 'strength'));
      aeroflyValues.wind.direction_in_degree = Number(_private.getValue(windMatch, 'direction_in_degree'));
      aeroflyValues.wind.turbulence  = Number(_private.getValue(windMatch, 'turbulence'));
      aeroflyValues.thermal_activity = Number( _private.getValue(windMatch, 'thermal_activity'));
    }

    // restrict this to `tmsettings_clouds`
    let cloudsMatch = _private.getGroup(configFileContent, 'tmsettings_clouds', 2);
    if (cloudsMatch) {
      aeroflyValues.clouds = ['cirrus', 'cumulus', 'cumulus_mediocris'].reverse().map((cloud) => {
        return {
          height:  Number(_private.getValue(cloudsMatch, cloud + '_height')),
          density: Number(_private.getValue(cloudsMatch, cloud + '_density'))
        };
      });
    }
