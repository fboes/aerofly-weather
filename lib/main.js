...
const main = function(verbose = false) {
  ...
  _private.processMetarObject = function(metarObject, destinationFile, hourOffset = 0, time = null, date = null) {
    try {
      ...
      if (verbose) {
        const flightplan = a.getFlightplan();
        console.log(
          "Aerofly flightplan\n------------------------\n",
          flightplan,
          "\n",
          (metarObject.icao === flightplan.origin.icao || metarObject.icao === flightplan.destination.icao)
            ? "Flightplan does match requested METAR"
            : "Flightplan does NOT match requested METAR"
          ,
          "\n"
        );
      }
