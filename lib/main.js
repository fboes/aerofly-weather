...
const main = function(verbose = false) {
  ...
  _private.processMetarObject = function(metarObject, destinationFile, options) {
    try {
      ...
      const aeroflyObject = metarToAerofly({ hourOffset: options.hourOffset || 0 }).convert(metarObject);
      if (verbose) {
  ...
  _public.fromUrl = function(url, destinationFile, options) {
    ...
    try {
      ...
      fetchMetarUrl(url, options, (response) => {
        if (verbose) {
          console.log('Got response from URL ' + url + "\n", response.trim(), "\n");
        }
        return _public.fromString(response, destinationFile, options);
      });
    } catch(error) {
      console.error(error.message || error);
      process.exit(1);
