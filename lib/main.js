...
const main = function(verbose = false) {
   ...
   * @returns {String}  ICAO code of destination airport in current flightplan
   */
  _private.getIcaoDestinationFromFlightplan = function(destinationFile) {
    const flightplan = aeroflyConfigFile(destinationFile).getFlightplan();
    if (!flightplan.destination && flightplan.destination.icao) {
      throw new Error('No flightplan found or no destination airport code found');
    }
    return flightplan.destination.icao || '';
  };
   ...
   */
  _private.processMetarObject = function(
    metarObject, destinationFile, hourOffset = 0, time = null, date = null, flightplan = null
  ) {
    try {
      if (verbose) {
   ...
   * @returns {Boolean} true
   */
  _public.fromString = function(
    metarCode, destinationFile, hourOffset = 0, time = null, date = null, flightplan = null
  ) {
    const metarObject   = metarParser(metarCode);
    return _private.processMetarObject(metarObject, destinationFile, hourOffset, time, date, flightplan);
  };
   ...
   * @returns {Boolean} true
   */
  _public.fromUrl = function(url, destinationFile, hourOffset = 0, time = null, date = null, flightplan = null) {
    url = url.trim().replace(/XXXX/, () => {
      return _private.getIcaoDestinationFromFlightplan(destinationFile);
    });
    ...
    client.get(url, (response) => {
      ...
      response.on('end', () => {
        ...
        return _public.fromString(rawData, destinationFile, hourOffset, time, date, flightplan);
      });
    }).on('error', (e) => {
   ...
   * @returns {Void} none
   */
  _public.fromCheckwx = function(
    apikey, icao, destinationFile, hourOffset = 0, time = null, date = null, flightplan = null
  ) {
    if (icao === 'XXXX') {
      icao = _private.getIcaoDestinationFromFlightplan(destinationFile);
    }
