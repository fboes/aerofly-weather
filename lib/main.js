...
const main = function(verbose = false) {
  ...
  _public.fromUrl = function(url, destinationFile, hourOffset = 0, time = null, date = null, flightplan = null) {
    url = url.trim().replace(/(XXXX|YYYY)/, (icao) => {
      return _private.getIcaoFromFlightplan(destinationFile, icao === 'XXXX');
    });
  ...
  _public.fromCheckwx = function(
    ...
    if (icao === 'XXXX' || icao === 'YYYY') {
      icao = _private.getIcaoFromFlightplan(destinationFile, icao === 'XXXX');
    }
