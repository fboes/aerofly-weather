 ...
 * * maxVisibility:    30000, // m  = 100% Aerofly
...
const metarToAerofly = function(metarObject, options = {}) {
  ...
  options.maxVisibility     = Number(options.maxVisibility)    || 30000; // m
   ...
   * @returns {Object} wind
   */
  _public.setWind = function(degrees, speed_kts, gust_kts) {
    ...
    _private.aeroflyValues.wind = {
      ...
      turbulence: _private.percent((gust_kts - speed_kts) / options.maxTurbulence, true)
    };
  ...
  _public.setThermalActivity = function(celsius) {
    _private.aeroflyValues.thermal_activity = _private.percent(
      celsius / options.maxTemperature, true
    );
  ...
  _public.convert = function(metarObject) {
    ...
    if (metarObject.visibility !== undefined && metarObject.visibility.meters !== undefined) {
      _public.setVisibility(metarObject.visibility.meters);
    }
    ...
    if (metarObject.temperature !== undefined && metarObject.temperature.celsius !== undefined) {
      _public.setThermalActivity(metarObject.temperature.celsius);
    }
