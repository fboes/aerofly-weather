 ...
 */
const metarToAerofly = function(options = {}) {
  ...
  _public.setDate = function(dateObject) {
    _private.aeroflyValues.time = {
      ...
      hour:   _private.getRotationalValue(dateObject.getUTCHours() + options.hourOffset, 24),
   ...
   * @returns {Object} wind
   */
  _public.setWind = function(degrees, speed_kts, gust_kts, degrees_from, degrees_to, conditions) {
    ...
    if (gust_kts === speed_kts) {
      gust_kts = _private.makeGust(speed_kts, conditions);
    }
  ...
  _public.setValue = function(key, value) {
    ...
    value = Number(value);
    if (keys.length === 3) {
      _private.aeroflyValues[keys[0]][keys[1]][keys[2]] = value;
    } else if (keys.length === 2) {
      _private.aeroflyValues[keys[0]][keys[1]] = value;
    } else {
      _private.aeroflyValues[key] = value;
    }
    return value;
  };
  ...
  _public.convert = function(metarObject) {
    ...
    _public.setWind(
      metarObject.wind ? metarObject.wind.degrees   : 0,
      metarObject.wind ? metarObject.wind.speed_kts : 0,
      metarObject.wind ? metarObject.wind.gust_kts  : 0,
      metarObject.wind ? metarObject.wind.degrees_from : 0,
      metarObject.wind ? metarObject.wind.degrees_to   : 0,
    ...
    // enforce 3 clouds
    let cloudLength = _private.aeroflyValues.clouds.length;
    while (cloudLength < 3) {
      _private.aeroflyValues.clouds.push({
        height: 0,
        density: 0
      });
      cloudLength++;
  ...
  /**
   * Get current state of values
   * @returns {Object} _private.aeroflyValues
   */
  ...
  /**
   * Get current state of values
   * @param   {Object} aeroflyValues dito
   * @returns {Object} _private.aeroflyValues
   */
  _public.set = function(aeroflyValues) {
    return _private.aeroflyValues = aeroflyValues;
  };
